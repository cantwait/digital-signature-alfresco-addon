...
public class SigningWebScript extends DeclarativeWebScript {
	 ...
	 */
	private static final String BUNDLE_FILE_NAME = "alfresco.module.repo-amp.messages.digitalSigning-action-messages";
		...
		try {
			if (SigningConstants.KEY_TYPE_X509.equals(keyType)) {
			    ...
			    if (keyContentReader != null && ks != null && keyPassword != null) {
			        ...
			        //validate the alias as well
			        if (c != null && keyAlias != certificateAlias) {
			        	...
			        	final Locale locale = (Locale) nodeService.getProperty(keyNodeRef, ContentModel.PROP_LOCALE);
			        	final ResourceBundle bundle = ResourceBundle.getBundle(BUNDLE_FILE_NAME, locale);
						final String errorMsg = bundle.getString("dgtsgn_model.msg.error");
			        	keyInfoDTO.setError(errorMsg);
			        }
